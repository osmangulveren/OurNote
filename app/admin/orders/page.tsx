  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <ViewModeToggle mode={view} baseUrl="/admin/orders" searchParams={{}} />
      </div>
      {orders.length === 0 ? (
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {orders.map((o) => {
        ...
        return (
          <Link key={o.id} href={`/admin/orders/${o.id}`} className="bg-white border border-slate-200 rounded-2xl overflow-hidden block hover:border-slate-400 transition">
            ...
            <div className="p-4 space-y-1">
              ...
              <p className="text-xs text-slate-500 truncate">{o.user.customerProfile?.companyName ?? o.user.email}</p>
              <p className="text-xs text-slate-500">
                {o.items.length} kalem · {o.createdAt.toLocaleDateString("tr-TR")}
              </p>
        ...
        <tbody>
          {orders.map((o) => (
            <tr key={o.id} className="border-t border-slate-100">
              <td className="px-4 py-2"><Link href={`/admin/orders/${o.id}`} className="hover:underline font-medium font-mono text-xs">{o.orderNumber}</Link></td>
              <td className="px-4 py-2 text-slate-600 text-xs">{o.createdAt.toLocaleString("tr-TR")}</td>
              <td className="px-4 py-2">{o.user.customerProfile?.companyName ?? o.user.email}</td>
